...
class JsArrayPatcher {
   ...
   @PatchMethod
   static void set(JsArray<JavaScriptObject> array, int index, JavaScriptObject value) {
      List<JavaScriptObject> wrapped = getWrappedList(array);
   ...
   private static <T extends JavaScriptObject> List<T> getWrappedList(JsArray<T> array) {

      List<T> wrappedList = (List<T>) JavaScriptObjects.getObject(array, JSARRAY_WRAPPED_LIST);
