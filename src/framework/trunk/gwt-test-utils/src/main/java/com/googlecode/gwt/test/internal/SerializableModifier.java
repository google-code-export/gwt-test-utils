 ...
 * 
 * <p>
 * To simulate this behaviour, a lot of work is done behind the scene by this
 * class. While it is loading a class, the {@link GwtClassLoader} knows if the
 * class is attempt to be compiled in JavaScript. If it is and if it can be
 * serialized (by implementing {@link IsSerializable} or {@link Serializable}
 * interface), it does a lot of bytecode manipulation (through the internal
 * {@link SerializableModifier} class) :
 * <ul>
 * <li>it replaces the potential {@link IsSerializable} reference to
