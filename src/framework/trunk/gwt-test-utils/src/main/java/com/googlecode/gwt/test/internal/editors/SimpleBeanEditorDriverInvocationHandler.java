 ...
 * 
 */
public class SimpleBeanEditorDriverInvocationHandler implements InvocationHandler {

   private static FieldFilter EDITOR_FIELD_FILTER = new FieldFilter() {

      public boolean matches(Field field) {
         return isEditorAssignable(field.getType()) && !Modifier.isPrivate(field.getModifiers())
   ...
   private static boolean isEditorAssignable(Class<?> candidate) {
      return Editor.class.isAssignableFrom(candidate) || IsEditor.class.isAssignableFrom(candidate);
   }
   ...
   private void collectEditorFields(final Map<String, LeafValueEditor<?>> result,
            final Editor<?> currentEditor, final String pathPrefix) {

      GwtReflectionUtils.doWithFields(currentEditor.getClass(), new FieldCallback() {
   ...
   private void collectEditorMethods(final Map<String, LeafValueEditor<?>> result,
            final Editor<?> currentEditor, final String pathPrefix) {

      GwtReflectionUtils.doWithMethods(currentEditor.getClass(), new MethodCallback() {
   ...
   private void edit(Object bean) throws IllegalAccessException, InvocationTargetException,
   ...
   private Object flush() throws IllegalAccessException, InvocationTargetException {
      if (this.bean == null) {
         throw new IllegalStateException(proxiedClass.getName()
   ...
   private Editor<?> getEditor(Editor<?> currentEditor, Field field) {
      if (Editor.class.isAssignableFrom(field.getType())) {
         return GwtReflectionUtils.<Editor<?>> getPrivateFieldValue(currentEditor, field);
      } else {
   ...
   private Editor<?> getEditor(Editor<?> currentEditor, Method method) {
      if (Editor.class.isAssignableFrom(method.getReturnType())) {
         return GwtReflectionUtils.<Editor<?>> callPrivateMethod(currentEditor, method);
      } else {
