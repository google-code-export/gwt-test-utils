...
public class CsvRunner {
	...
	public void executeLine(String methodName, List<String> args, Object fixture)
		...
		if (methodName.indexOf("**") != 0) {
			...
			logger.debug(getProcessingMessagePrefix() + "Executing "
					+ methodName + ", params "
			...
			try {
				...
				logger.error(getAssertionErrorMessagePrefix()
						+ "Execution error", e);
	...
	public <T> T getNodeValue(Object o, Node node) {
		...
		while (currentNode != null) {
			...
			logger.trace(getProcessingMessagePrefix() + "Processing "
					+ currentName);
			boolean ok = false;
			if (!ok) {
				Method m = null;
				if (m == null) {
					m = GwtReflectionUtils.getMethod(current.getClass(),
							currentName);
				}
				if (m == null) {
				...
				if (m != null) {
					try {
						if (m.getParameterTypes().length == 0
							...
							current = invoke(current, m,
									currentNode.getParams());
							ok = true;
						} else {
							current = findInList(current, m,
									currentNode.getMapEq(),
									currentNode.getMap());
						...
						fail(getAssertionErrorMessagePrefix()
								...
								+ o.getClass().getCanonicalName() + ", method "
								+ m.getName() + ", params "
								+ currentNode.getParams());
					}
				}
	...
	@SuppressWarnings("unchecked")
	public <T> T getObject(Class<T> clazz, boolean failOnError,
			String... params) {
		...
		Assert.fail(getAssertionErrorMessagePrefix()
				+ "Wrong object type, not a " + clazz.getCanonicalName()
	...
	private Object findInList(final Object current, final Method m, Node mapEq,
			String map) throws IllegalArgumentException,
				...
				return new Iterator<Object>() {
					...
					public Object next() {
						try {
							...
							throw new GwtTestCsvException("Iterator exception",
									e);
						}
					}
	...
	private Method getCsvMethod(Class<?> clazz, String methodName) {
		...
		if (classCsvMethods == null) {
			...
			Map<Method, CsvMethod> csvMethods = GwtReflectionUtils
					.getAnnotatedMethod(clazz, CsvMethod.class);
	...
	private Object invoke(Object current, Method m, List<String> list)
		...
		logger.debug("Invoking " + m.getName() + " on "
				+ current.getClass().getCanonicalName() + " with param " + list);
		GwtReflectionUtils.makeAccessible(m);
		if (list == null) {
			if (m.getParameterTypes().length == 0) {
				try {
