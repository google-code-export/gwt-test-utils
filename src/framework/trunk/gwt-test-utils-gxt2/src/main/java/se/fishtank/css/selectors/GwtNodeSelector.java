 ...
 * case. If it's possible to make this work somehow we could speed this up a
 * bit. Maybe!?
 */
public class GwtNodeSelector implements NodeSelector<Node> {
   ...
   * {@inheritDoc}
   */
  public Set<Node> querySelectorAll(String selectors)
  ...
  /**
   * Check the list of selector <em>parts</em> and return a set of nodes with
   * the result.
   * 
      ...
      if (selector.hasSpecifiers()) {
        for (Specifier specifier : selector.getSpecifiers()) {
          switch (specifier.getType()) {
            case ATTRIBUTE:
              checker = new AttributeSpecifierChecker(
                  (AttributeSpecifier) specifier);
              break;
            case PSEUDO:
              if (specifier instanceof PseudoClassSpecifier) {
                checker = new PseudoClassSpecifierChecker(
                    (PseudoClassSpecifier) specifier);
              } else if (specifier instanceof PseudoNthSpecifier) {
                checker = new PseudoNthSpecifierChecker(
                    (PseudoNthSpecifier) specifier);
              }
   ...
   * <p/>
   * This method will add the {@link Selector} from the specifier in a list and
   * invoke {@link #check(List)} with that list as the argument.
   * 
