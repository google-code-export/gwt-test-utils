 ...
 * {@linkplain Selector#getTagName() tag name} and
...
public class TagChecker extends NodeTraversalChecker {
	 ...
	 *             If one of the nodes have an illegal type.
	 */
	...
	private void addGeneralSiblingElements() {
		for (Node node : nodes) {
			Node n = DOMHelper.getNextSiblingElement(node);
			while (n != null) {
				if (selector.getTagName().equals(n.getNodeName()) || selector.getTagName().equals(Selector.UNIVERSAL_TAG)) {
					result.add(n);
				}
