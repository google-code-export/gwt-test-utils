...
public class DOMNodeSelector implements NodeSelector<Node> {
	 ...
	 * 
	 * @param root
	 *            The root node. Must be a document or element node.
	 */
	public DOMNodeSelector(Node root) {
		Assert.notNull(root, "root is null!");
		short nodeType = root.getNodeType();
		Assert.isTrue(nodeType == Node.DOCUMENT_NODE || nodeType == Node.ELEMENT_NODE, "root must be a document or element node!");
		this.root = root;
	}
	 ...
	 * 
	 * @param parts
	 *            A list of selector <em>parts</em>.
	 ...
	 * <p/>
	 * This method will add the {@link Selector} from the specifier in a list
	 * and invoke {@link #check(List)} with that list as the argument.
	 * 
	 * @param specifier
	 *            The negation specifier.
