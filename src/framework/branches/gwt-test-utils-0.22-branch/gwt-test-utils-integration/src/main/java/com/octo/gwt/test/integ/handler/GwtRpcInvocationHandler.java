  ...
  public Object invoke(Object proxy, Method method, Object[] args)
    ...
    try {
      ...
      for (int i = 0; i < subArgs.length; i++) {
        serializedArgs[i] = GwtSerializationUtils.serializeUnserialize(
            subArgs[i], GwtTestClassLoader.getInstance().getParent(),
            fromGwtCallbacks);
      }
      Object returnValue = m.invoke(target, serializedArgs);
      Object o = GwtSerializationUtils.serializeUnserialize(returnValue,
          GwtTestClassLoader.getInstance(), backToGwtCallbacks);
