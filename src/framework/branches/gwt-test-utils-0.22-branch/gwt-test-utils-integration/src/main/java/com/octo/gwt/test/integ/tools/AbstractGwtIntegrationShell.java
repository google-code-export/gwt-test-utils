...
public abstract class AbstractGwtIntegrationShell extends
    AbstractGwtConfigurableTest {
  ...
  public void clickComplexPanel(String index, String... params) {
    ...
    Event complexPanelClick = EventBuilder.create(Event.ONCLICK).setTarget(
        target.getElement()).build();
    assertCanApplyEvent(target, complexPanelClick.getTypeInt());
    dispatchNotCheckedEvent(
        panel,
        EventBuilder.create(Event.ONCLICK).setTarget(target.getElement()).build());
  }
  ...
  protected void assertCanApplyEvent(UIObject widget, int eventTypeInt) {
    if (!WidgetUtils.isWidgetVisible(widget)) {
      String failureMessage = createFailureMessage(widget, eventTypeInt,
          "visible");
