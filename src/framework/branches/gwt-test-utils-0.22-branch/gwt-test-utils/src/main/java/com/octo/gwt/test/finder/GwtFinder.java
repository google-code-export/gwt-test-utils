...
public class GwtFinder implements AfterTestCallback {
  ...
  private class DefaultObjectFinder implements ObjectFinder {
    ...
    public Object find(String... params) {
      ...
      if (node == null) {
        throw new GwtFinderException(
            "Cannot parse search string to a valid Node : " + params[0]);
      }
      ...
      if (finder == null) {
        throw new GwtFinderException("Unknown node prefix '" + prefix
            + "'. You must register your own "
            + NodeObjectFinder.class.getSimpleName()
            + " instance for this prefix through the "
            + GwtFinder.class.getSimpleName()
  ...
  protected static void onAttach(Widget widget) {
    ...
    if (HasHTML.class.isInstance(widget)) {
      INSTANCE.indexedObjectFinder.repository.put(((HasHTML) widget).getHTML(),
          widget);
    }

    if (HasText.class.isInstance(widget)) {
      INSTANCE.indexedObjectFinder.repository.put(((HasText) widget).getText(),
          widget);
    }

    if (HasName.class.isInstance(widget)) {
      INSTANCE.indexedObjectFinder.repository.put(((HasName) widget).getName(),
          widget);
    }
   ...
   * others internal patchers
   */
  protected static void onSetHTML(HasHTML hasHTML, String newHTML,
      String oldHTML) {
   ...
   * others internal patchers
   */
  protected static void onSetName(HasName hasName, String newName,
      String oldName) {
   ...
   * others internal patchers
   */
  protected static void onSetText(HasText hasText, String newText,
      String oldText) {
  ...
  private Object findInIterable(Iterable<Object> list, Node before,
    ...
    for (Object n : list) {
      if (checkCondition(n, before, after)) {
        if (found != null) {
          throw new GwtFinderException("Not unique object with condition "
              + before + "=" + after);
        }
        found = n;
      }
  ...
  private Object findInList(final Object current, final Method m, Node mapEq,
    ...
    if (m.getParameterTypes().length != 1
      ...
      throw new GwtFinderException("Unable to navigate "
          + current.getClass().getCanonicalName() + " with method "
          + m.getName());
    }
  ...
  private <T> T findInternal(Object o, Node node) {
    ...
    while (currentNode != null) {
      ...
      if (!ok) {
        ...
        if (m == null) {
          m = GwtReflectionUtils.getMethod(current.getClass(), "get"
              + currentName);
        }
        if (m != null) {
          try {
            if (m.getParameterTypes().length == 0
                || currentNode.getParams() != null) {
              current = invoke(current, m, currentNode.getParams());
              ok = true;
            } else {
              current = findInList(current, m, currentNode.getMapEq(),
                  currentNode.getMap());
  ...
  private <T> T findInternal(String... params) {
    ...
    throw new GwtFinderException("No registered "
        + ObjectFinder.class.getSimpleName()
        + " instance can be used to find object with param: "
  ...
  private Object invoke(Object current, Method m, List<String> list)
    ...
    for (int index = 0; index < m.getParameterTypes().length; index++) {
      if (m.getParameterTypes()[index] == String.class) {
        ...
        throw new GwtFinderException("Error while calling method "
            + m.toGenericString() + ": Not managed type "
  ...
  private Object proccessMap(Object current, String map) {
    if (current instanceof Map<?, ?>) {
      ...
      throw new GwtFinderException("Object not a map "
