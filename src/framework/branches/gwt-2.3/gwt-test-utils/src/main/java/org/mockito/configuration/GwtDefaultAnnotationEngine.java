...
public class GwtDefaultAnnotationEngine extends DefaultAnnotationEngine {
  ...
  public Object createMockFor(Annotation annotation, Field field) {
    ...
    if (annotation instanceof com.googlecode.gwt.test.Mock) {
      return processAnnotationOn((com.googlecode.gwt.test.Mock) annotation,
          field);
    }
    ...
    if (GwtFactory.get().getOverlayRewriter().isJsoIntf(f.getType().getName())) {
      try {
        ...
        throw new GwtTestPatchException(
            "Error while creating a mock with Mockito for "
                + f.getType().getName(), e);
      }
    } else {
      return f.getType();
  ...
  private Object processAnnotationOn(com.googlecode.gwt.test.Mock annotation,
      Field field) {
  ...
  @SuppressWarnings("deprecation")
  private Object processAnnotationOn(
      org.mockito.MockitoAnnotations.Mock annotation, Field field) {
    return Mockito.mock(getTypeToMock(field), field.getName());
  }
