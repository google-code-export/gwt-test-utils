...
public abstract class LocalizableResourcesInvocationHandler implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		throw new GwtTestI18NException("Unable to find a Locale specific resource file to bind with i18n interface '" + proxiedClass.getName()
				+ "' and there is no @DefaultXXXValue annotation on '" + method.getName() + "' called method");
	}

	private Locale getResourceLocale(Class<?> clazz) {
		if (GwtConfig.getLocale() != null) {
			return GwtConfig.getLocale();
		}
		...
		if (annotation != null) {
			...
			default:
				throw new GwtTestI18NException("Cannot parse Locale value in annoted class [" + clazz.getSimpleName() + "] : @"
