...
public class CurrencyListPatcher {
	...
	private static class CurrencyDataHolder implements AfterTestCallback {
		...
		private CurrencyData createCurrencyData(Locale locale) {
			Properties currencyData = GwtPropertiesHelper
					.get()
			...
			Properties numberConstants = GwtPropertiesHelper
					.get()
					.getLocalizedProperties(
			...
			String defCurrencyCode = numberConstants
					.getProperty("defCurrencyCode");
			if (defCurrencyCode == null) {
				defCurrencyCode = "USD";
			}

			CurrencyData defCurrencyData = new CurrencyDataImpl(
					defCurrencyCode, defCurrencyCode, 2, "");
			...
			for (String currencyCode : currencies) {
				...
				if (currencySplit.length > 2 && currencySplit[2].length() > 0) {
					try {
						currencyFractionDigits = Integer
								.valueOf(currencySplit[2]);
				...
				if (extraData != null) {
					...
					// If a non-empty override is supplied, use it for the
					// currency
					// symbol.
