...
public class Browser {
	...
	public static void click(ComplexPanel panel, int index) {
		...
		Event clickEvent = EventBuilder.create(Event.ONCLICK)
				.setTarget(target.getElement()).build();
	...
	public static void click(MenuBar parent, MenuItem clickedItem) {
		Event clickEvent = EventBuilder.create(Event.ONCLICK)
				.setTarget(clickedItem.getElement()).build();
	...
	public static void emptyText(HasText hasTextWidget) {
		...
		for (int i = 0; i < baseLength; i++) {
			Event keyDownEvent = EventBuilder.create(Event.ONKEYDOWN)
					.setKeyCode(KeyCodes.KEY_BACKSPACE).build();
		...
		// before
		Event keyUpEvent = EventBuilder.create(Event.ONKEYUP)
				.setKeyCode(KeyCodes.KEY_BACKSPACE).build();
	...
	public static void fillText(HasText hasTextWidget, String value) {
		...
		for (int i = 0; i < value.length(); i++) {
			...
			Event keyPressEvent = EventBuilder.create(Event.ONKEYPRESS)
					.setKeyCode(keyCode).build();
	...
	public static void keyDown(Widget target, int keyCode) {
		dispatchEvent(target,
				EventBuilder.create(Event.ONKEYDOWN).setKeyCode(keyCode)
						.build());
	}
	...
	public static void keyPress(Widget target, int keyCode) {
		dispatchEvent(target,
				EventBuilder.create(Event.ONKEYPRESS).setKeyCode(keyCode)
						.build());
	}
	 ...
	 */
	public static void removeText(HasText hasTextWidget,
			int backspacePressNumber) {
	...
	private static String createFailureMessage(UIObject target, Event event,
		...
		sb.append(" to apply a browser '").append(event.getType())
				.append("\' event");
