...
public abstract class GwtTestWithEasyMock extends GwtTest {
	...
	private final Map<Class<?>, Object> mockObjects = new HashMap<Class<?>, Object>();

	private final List<Class<?>> mockedClasses = new ArrayList<Class<?>>();

	private final Set<Field> annotatedFieldToInject;

	public GwtTestWithEasyMock() {
		GwtCreateHandlerManager.get().setMockCreateHandler(
				new MockCreateHandler(mockObjects));
		annotatedFieldToInject = GwtReflectionUtils.getAnnotatedField(
				this.getClass(), Mock.class).keySet();
	...
	public <T> T createMockAndKeepMethods(Class<T> clazz,
		...
		T o = EasyMock.createMockBuilder(clazz)
				.addMockedMethods(l.toArray(new Method[] {})).createMock();
