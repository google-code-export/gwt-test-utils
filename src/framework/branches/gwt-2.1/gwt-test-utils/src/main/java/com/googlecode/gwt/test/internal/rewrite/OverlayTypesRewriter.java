...
public class OverlayTypesRewriter {
  ...
  private class MyInstanceMethodOracle implements InstanceMethodOracle {
    ...
    private MyInstanceMethodOracle(Set<JClassType> jsoTypes,
   ...
   */
  public OverlayTypesRewriter(CompilationState compilationState,
      JClassType jsoType) {
    ...
    // FIXME: MatchResult extends JavaScriptObject at runtime, don't know why...
    notJsoDesc = toDescriptor("com.google.gwt.regexp.shared.MatchResult");
    buildJsoIntfDescs.remove(notJsoDesc);
    buildJsoImplDescs.remove(notJsoDesc);
    buildJsoSuperDescs.remove(notJsoDesc);
    // FIXME: SplitResult extends JavaScriptObject at runtime, don't know why...
    ...
    this.jsoData = new MySingleJsoImplData();
    this.mapper = new MyInstanceMethodOracle(jsoTypes,
        typeOracle.getJavaLangObject(), jsoData);
  }
  ...
  public byte[] rewrite(String className, byte[] classBytes) {
    Event classBytesRewriteEvent = SpeedTracerLogger.start(
        DevModeEventType.CLASS_BYTES_REWRITE, "Class Name", className);
