...
import com.octo.gwt.test.csv.tools.NodeObjectFinderFactory;
...
public abstract class GwtCsvTest extends GwtTest implements
		NodeObjectFinderFactory {

	private static class MacroReader {

		private final static Logger logger = LoggerFactory
				.getLogger(MacroReader.class);
			...
			for (List<String> row : sheet) {
				if (currentMacroName != null) {
					if (row.size() > 0 && "endmacro".equals(row.get(0))) {
						logger.info("End macro, length : "
								+ currentMacroContent.size());
					...
					if (row.size() > 0 && "macro".equals(row.get(0))) {
						...
						Assert.assertNotNull(
								"You have to specified a macro name",
								currentMacroName);
	...
	// MODE INTERACTIF
	private static final Class<?>[] baseList = { String.class, Integer.class,
	...
	public void assertExact(String value, String... params) {
		...
		if (value == null) {
			Assert.assertNull(csvRunner.getAssertionErrorMessagePrefix()
					+ "Null was expected, but '" + actualValue
					+ "' has been find", actualValue);
		} else {
	...
	public final NodeObjectFinder createNodeObjectFinder(String prefix) {
		return getNodeObjectFinder(prefix);
	}
	...
	@CsvMethod
	public void fillAndSelectInSuggestBoxByText(String content,
			String selected, String... params) {
		SuggestBox suggestBox = getObject(SuggestBox.class, params);
		Browser.fillText(suggestBox, content);
		...
		while (i < menuItems.size() && index == -1) {
			...
			if (selected.equals(item.getHTML())
					|| selected.equals(item.getText())) {
				index = i;
		...
		Assert.assertTrue(csvRunner.getAssertionErrorMessagePrefix()
				+ "Cannot find '" + selected + "' in suggested choices",
				index > -1);
	}
	...
	public void hasStyle(String style, String... params) {
		...
		Assert.assertTrue(csvRunner.getAssertionErrorMessagePrefix()
				+ "Style not found : " + style,
				object.getStyleName().contains(style));
	}
	...
	public void isEnabled(String... params) {
		...
		Assert.assertTrue(csvRunner.getAssertionErrorMessagePrefix()
				+ "targeted " + target.getClass().getSimpleName()
				+ " is not enabled", target.isEnabled());
	}
	...
	public void isVisible(String... params) {
		...
		Assert.assertTrue(csvRunner.getAssertionErrorMessagePrefix()
				+ "targeted " + object.getClass().getSimpleName()
				+ " is not visible", WidgetUtils.isWidgetVisible(object));
	...
	public void setReader(DirectoryTestReader reader) {
		...
		ObjectFinderManager.get().setup(csvRunner, this, getWidgetVisitor());
	}
	...
	protected String getString(Object o) {
		...
		if (o == null) {
			...
			actualValue = (html != null && html.length() > 0) ? html : hasHTML
					.getText();
	...
	private void printGetter(Object o, Class<?> clazz, PrintStream os) {
		for (Method m : clazz.getDeclaredMethods()) {
			if (m.getName().startsWith("get")) {
				os.print("Getter [" + clazz.getSimpleName() + "] "
						+ m.getName());
