...
class MessagesInvocationHandler extends LocalizableResourceInvocationHandler {
   ...
   * args, the corresponding values are appended with the '|' separator.
   * 
  ...
  private String extractPluralCountAndSelectValues(
      Annotation messageAnnotation, Method method, Object[] args, Locale locale) {
      ...
      for (int j = 0; j < childArray.length; j++) {
        if (PluralCount.class.isAssignableFrom(childArray[j].getClass())) {
          ...
          try {
            ...
            sb.append(
                ruleInstance.pluralForms()[ruleInstance.select(count)].getName()).append(
                "|");
  ...
  private String getSpecificKey(Annotation specificMessageAnnotation,
      Method method, Object[] args, Locale locale) {
    String pluralCountValue = extractPluralCountAndSelectValues(
