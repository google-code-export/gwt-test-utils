...
public class GwtParanamer {
  ...
  private static class GwtTestJavaFileFinder implements JavaFileFinder {
    ...
    private InputStream getInputStream(Class<?> declaringClass) {
      ...
      // file is in a 3rd party GWT module (e.g. a .jar with .java files in it)
      InputStream is = declaringClass.getClassLoader().getResourceAsStream(
          javaPath);
  ...
  private static final Paranamer INSTANCE = new JavaFileParanamer(
      new GwtTestJavaFileFinder());
